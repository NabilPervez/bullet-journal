...
//
//   npm run twa:check
//   npm run twa:check -- https://deploy-preview-12--bullet-journal-app.netlify.app
//
// Exit code is 1 if anything required is wrong, so it can gate a release.

const DEFAULT_ORIGIN = "https://bullet-journal-app.netlify.app";
const EXPECTED_PACKAGE = "com.nabilpervezconsulting.bulletjournal";
...
// ---------------------------------------------------------------------------
section("1. HTTPS");
if (!origin.startsWith("https://")) bad("Origin must be https:// — a TWA will not verify over plain HTTP");
...
if (sw.res?.ok && /javascript/.test(sw.type)) {
  ...
  if (/privacy/.test(body)) ok("Service worker leaves /privacy to the network");
  else warn("Service worker may answer /privacy with the app shell once installed");
...
if (!privacy.res?.ok) {
  bad(`/privacy answers ${privacy.res?.status}`);
} else {
  const body = await privacy.res.text();
  if (/Privacy Policy/i.test(body) && !/id="root"/.test(body)) ok(`${privacy.url} serves the policy  ← use this URL in Play Console`);
  else bad("/privacy answers with the app shell, not the policy");
  if (/REPLACE_WITH_CONTACT_EMAIL/.test(body)) bad("The privacy policy still has the contact email placeholder");
}
